...
@Getter
@Slf4j
public class MyDynamicKeyOpenAiAuthInterceptor extends OpenAiAuthInterceptor {
    /**
     * 账号被封了
     */
     ...
     *
     */
    public MyDynamicKeyOpenAiAuthInterceptor() {
     ...
     */
    public MyDynamicKeyOpenAiAuthInterceptor(Map warringConfig) {
        ...
        if (!response.isSuccessful()) {
            ...
            if (response.code() == CommonError.OPENAI_AUTHENTICATION_ERROR.code()
                ...
                if (ACCOUNT_DEACTIVATED.equals(errorCode) || INVALID_API_KEY.equals(errorCode)) {
                    ...
                    if ("gpt-3.5-turbo".equals(chatConfig.getOpenaiApiModel())){
                        chatConfig.setOpenaiApiKey3List(this.onErrorDealApiKeys(key));
                    }
                    else {
                        chatConfig.setOpenaiApiKey4List(this.onErrorDealApiKeys(key));
                    }
                }
                throw new BaseException(openAiResponse.getError().getMessage() + " dynamic error");
            }
            //非官方定义的错误code
            log.error("--------> 请求异常：{}", errorMsg);
            OpenAiResponse openAiResponse = JSONUtil.toBean(errorMsg, OpenAiResponse.class);
            ...
            throw new BaseException(CommonError.RETRY_ERROR + " dynamic error");
        }
        return response;
    }
